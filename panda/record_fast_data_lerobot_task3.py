...
if __name__ == '__main__':
    ...
    dataset_dir = Path("outputs/fast_pick_and_place_task3")
    
    if dataset_dir.exists():
        print("[*] Found existing dataset. Loading it to append a new episode...")
        dataset = LeRobotDataset("local/fast_pick_and_place_task3", root=dataset_dir)
    else:
        print("[*] Creating new dataset folder...")
        dataset = LeRobotDataset.create(
            repo_id="local/fast_pick_and_place_task3",
            root=dataset_dir,
            features={
    ...
    pose[2,3] -= 0.1 
    q = panda_py.ik(pose)
    start_hover_q = np.array(q, dtype=np.float64)
    panda.move_to_joint_position(start_hover_q, speed_factor=0.2)
    time.sleep(1)
    ...
    try:
        ...
        print("returning to start hover position")
        panda.move_to_joint_position(start_hover_q, speed_factor=0.1)
    ...
    if episode_successful:
        ...
        instruction = "pick up cube one and return it to the start position"
